...
//////////////////////////////////////////////////////////////////////////
	...
	void SetupShapesUserData(const VehicleDescriptor &vd)
	{
		...
		r3d_assert(numShapes <= vd.numWheels + vd.numHullParts + vd.numSeats + vd.numTurrets);
		//r3dOutToLog( "!!! Shapes: %d, Wheels: %d, Hull: %d, Seats: %d\n", numShapes, vd.numWheels, vd.numHullParts, vd.numSeats );
...
//////////////////////////////////////////////////////////////////////////
	...
	PxRigidDynamic * CreateVehicleActor(const PhysicsObjectConfig& PhysicsConfig, const VehicleDescriptor &vd, PxConvexMesh &hull, PxConvexMesh **wheels)
	{
		...
		(
			...
			vd.chassisData, &seatLocalPoses[0], vd.numSeats, &turretLocalPoses[0], vd.numTurrets
		);
...
void VehicleManager::UpdateInput()
{
	...
	if (isLocalVehicle && vehicle->IsCarDrivable())
	{
		...
		else
		{
			...
			bool isLeft = InputMappingMngr->isPressed(r3dInputMappingMngr::KS_VEHICLE_TURNRIGHT);
			bool isRight = InputMappingMngr->isPressed(r3dInputMappingMngr::KS_VEHICLE_TURNLEFT);
...
void VehicleCameraController::Update(float dtime)
{
	...
	PxVec3 rigOffset(CurrentRig.Position.x, CurrentRig.Position.y, ((isRearView) ? 1.0f : -1.0f) * CurrentRig.Position.z);
