...
// 		}
	...
	CanSprint = IsSuperZombie() ? false : ( spawnObject->m_SprintersSpawnPerc >= u_GetRandom(0.0f, 1.0f) );
...
{
	const float eyeHeight = ( IsSuperZombie() ) ? 2.4f : 1.6f;
#ifdef ZOMBIE_PERFORMANCE_TESTING
	float startTime = r3dGetTime();
...
bool obj_Zombie::StartAttack(const GameObject* trg)
{
	...
	SetNavAgentSpeed((IsSuperZombie() || staggerTime < 0) ? RunSpeed : 0.0f);
