...
{
	r3dOutToLog("!!! crashed, thread %d\n", GetCurrentThreadId());

	static volatile LONG alreadyCrashed = 0;
	if(alreadyCrashed)
	{
		r3dOutToLog("crash report in another thread - not generating\n");
		for(; InterlockedExchange(&alreadyCrashed, 1) == 1; ) ;
	...
	InterlockedExchange(&alreadyCrashed, 1);
	...
	InterlockedExchange(&alreadyCrashed, 0);
