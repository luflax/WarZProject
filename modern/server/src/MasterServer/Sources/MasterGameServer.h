...
class CMasterGameServer : public r3dNetCallback
{
  public:
	...
	DWORD		curSuperId_;
	bool		IsServerStartingUp();
	
	CServerS*	GetLeastUsedServer(EGBGameRegion region, ESBServerType serverType, int mapId);
	CServerS*	GetServerByGameId(DWORD gameId);
	CServerG*	GetGameByGameServerId(DWORD gameServerId);
	CServerG*	GetGameBySessionId(__int64 sessionId);
	CServerG*	GetQuickJoinGame(int gameMap,EGBGameRegion region, int BrowseChannel, int timePlayed);
	
	void		DeleteGame(CServerG* game);	// deregister and delete game
	void		DeleteAllSupervisorGames(DWORD superId);
	...
	std::vector<DWORD> blockedIps_;
	int		blockedAttempts_;
	...
	void		UpdateAllGames();
	bool		 UpdatePermanentGames();
	int		  IsGameServerIdStarted(DWORD gameServerId, CServerG** out_game);
	void		CloseExpiredGames();
	
	float		nextBillingHourUpdate_;
